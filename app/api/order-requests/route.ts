...
type IncomingBody = {
  company?: string; contactName?: string; phone?: string; city?: string; deliveryMethod?: string; comment?: string; items?: IncomingLine[]
}
...
export async function POST(request: Request) {
  try {
    ...
    const deliveryMethod = text(body.deliveryMethod,80)
    const comment = text(body.comment,1000)
    if (!company || !contactName || phone.replace(/\D/g,'').length < 8 || !city || !deliveryMethod) {
      return NextResponse.json({ ok:false, error:'invalid_contact_data' }, { status:400 })
    }
    ...
    const bySourceId = new Map(catalog.map(product => [product.sourceId, product]))
    const items = requested.flatMap(line => {
      const product = bySourceId.get(text(line.sourceId,120))
      const boxes = Math.min(100, Math.max(1, Math.floor(Number(line.boxes)||0)))
      if (!product || product.status !== 'active') return []
      return [{ source_id:product.sourceId, sku:product.sku, name:product.name, boxes, pack_size:product.packSize, unit_price:product.unitPrice, line_total:boxes*product.packSize*product.unitPrice }]
    })
