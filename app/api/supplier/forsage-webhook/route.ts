...
export async function POST(request: Request) {
  ...
  try {
    ...
    const productIds = Array.isArray(body.product_ids) ? body.product_ids.map(Number).filter(Number.isFinite).slice(0,5000) : []
    ...
    const { error } = await supabase.from('econom_supplier_events').insert({ product_ids:productIds, change_types:changeTypes, status:'pending' })
    if (error) {
      console.error('econom_forsage_event_insert_failed', error.code, error.message)
      return NextResponse.json({ ok:false, error:'queue_failed' }, { status:500 })
    }
    return NextResponse.json({ ok:true }, { status:202 })
  } catch (error) {
    console.error('econom_forsage_webhook_invalid', error)
