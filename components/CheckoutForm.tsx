...
export function CheckoutForm(){
 ...
 const currency=store.cartCurrency??store.lines[0]?.product.currency??'UAH'
 async function submit(e:React.FormEvent<HTMLFormElement>){
   e.preventDefault()
   if(!store.lines.length){setError('Добавьте хотя бы одну упаковку в корзину.');return}
   if(store.mixedCurrency){setError('Товары в гривне и долларах оформляются отдельными заявками. Вернитесь в корзину и оставьте одну валюту.');return}
   ...
   try{
     ...
     if(!response.ok||!result.ok){
       if(result.error==='mixed_currency')setError('Товары в гривне и долларах оформляются отдельными заявками.')
       else if(result.error==='catalog_changed')setError('Каталог изменился. Вернитесь в каталог и добавьте товары заново.')
       else setError('Не удалось отправить заявку. Проверьте данные и попробуйте ещё раз.')
       return
     }
     if(result.persisted) store.clear()
     setDone(result)
   } catch(err){setError('Не удалось отправить заявку. Проверьте соединение и попробуйте ещё раз.');console.error(err)} finally{setSending(false)}
 }
 ...
 return <div className="checkout-layout"><form className="checkout-form" onSubmit={submit}><div><span className="eyebrow">Оптовый заказ</span><h1>Контакты покупателя</h1><p>Заказ сначала подтверждает менеджер, потому что фактическое наличие отдельных моделей проверяется перед оплатой.</p></div>{store.mixedCurrency&&<div className="checkout-warning"><AlertCircle size={17}/><span>В корзине сохранены товары в разных валютах. Оформите гривневые и долларовые позиции отдельными заявками.</span></div>}<div className="form-grid"><label>Компания / магазин<input name="company" required maxLength={120} autoComplete="organization" placeholder="Название магазина"/></label><label>Контактное лицо<input name="name" required maxLength={100} autoComplete="name" placeholder="Имя"/></label><label>Телефон<input name="phone" required maxLength={40} autoComplete="tel" inputMode="tel" placeholder="+380 __ ___ __ __"/></label><label>Город<input name="city" required maxLength={100} autoComplete="address-level2" placeholder="Одесса"/></label><label>Доставка<select name="delivery" required defaultValue=""><option value="" disabled>Выберите</option><option>Новая почта</option><option>Укрпочта</option><option>Самовывоз</option></select></label><label className="wide">Комментарий<textarea name="comment" maxLength={1000} placeholder="Уточнения по заказу"/></label></div>{error&&<p className="form-error">{error}</p>}<button className="primary full" disabled={sending||!store.lines.length||store.mixedCurrency}>{sending?'Отправляем…':'Отправить заявку менеджеру'}</button></form><aside className="order-summary"><small>Ваш заказ</small><h2>{store.boxCount} уп.</h2>{store.lines.map(l=><div className="summary-line" key={l.product.id}><span>{l.product.name}<small>{l.boxes} × {l.product.packSize} шт.</small></span><b>{money(l.product.unitPrice*l.product.packSize*l.boxes,l.product.currency??'UAH')}</b></div>)}<div className="summary-total"><span>Итого</span><strong>{store.mixedCurrency?'Разные валюты':money(store.total,currency)}</strong></div><p>Цены и валюта берутся из каталога поставщика. Финальная сумма и наличие подтверждаются менеджером до предоплаты.</p></aside></div>
}
