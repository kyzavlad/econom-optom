...
function dbToProduct(row: DbProduct): Product {
  ...
  return {
    ...
    season: row.season ?? 'Не указано',
    gender: row.gender ?? 'Не указано',
...
export async function getProducts(): Promise<Product[]> {
  ...
  const { data, error } = await supabase.from('econom_products').select(selectFields).eq('is_active', true).order('created_at', { ascending: false })
...
export async function getProductBySlug(slug: string): Promise<Product | null> {
  const supabase = serverClient()
  if (supabase) {
    const { data, error } = await supabase.from('econom_products').select(selectFields).eq('slug', slug).eq('is_active', true).maybeSingle()
    if (!error && data) return dbToProduct(data as unknown as DbProduct)
  }
